...
def train_model(model, name=None, image_size=32, early_stopping_min=1, epochs_min=20, path=None):
    ...
    dataset = DataGenerator(x_train, y_train_onehot, batch_size=32, shuffle=True, augment=False)
...
grid_dropout = [0.1, 0.25, 0.5]
grid_kernel = [3, 4]
grid_layers = [2, 3, 4]
grid_nodes = [128, 256, 512]
...
for i in range(20):
    ...
    path = 'model_lr{}_dr{}_ks{}_ly{}_nd{}'.format(lr, drop_rate, kernel, nlayers, node)
    if os.path.exists(path):
        continue
    model = build_SV_GAP(learning_rate=lr, ksize=kernel, drop_rate=drop_rate,
    nlayers=nlayers, node=node)
    train_model(model=model, name=path, image_size=32, early_stopping_min=10, epochs_min=60,path=path)
